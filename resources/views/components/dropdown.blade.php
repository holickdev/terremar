...
@php
    $baseClass =
        'inline-flex items-center justify-center gap-x-1.5 rounded-md px-3 py-2 text-sm font-semibold shadow-sm text-gray-300 hover:bg-gray-700 hover:text-white';
    $iconClass =
...
<div x-data="{ open: false }" class="relative inline-block text-left">
    ...
    <div>
        <button @click="open = !open" type="button" class="{{ $icon ? $iconClass : $baseClass }}" id="menu-button"
        ...
        <div class="py-1" role="none">
            @foreach ($options as $option)
                <a href="{{ $option['url'] ?? '#' }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                    role="menuitem" tabindex="-1" @click="open = false"> <!-- Cierra el menú al hacer clic -->
                    {{ $option['label'] }}
                </a>
            @endforeach
